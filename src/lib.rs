...
impl<'a> CobsEncoder<'a> {
    ...
    pub fn push(&mut self, data: &[u8]) -> Result<(), ()> {
        // TODO: could probably check if this would fit without iterating
        // through all data
        for x in data {
            if *x == 0 {
    ...
    /// Complete encoding of the output message. Does NOT terminate the message
    ...
    pub fn finalize(self) -> Result<usize, ()> {
        ...
        // If the current code index is outside of the destination slice, we do
        // not need to write it out
...
///
/// This function uses the typical sentinel value of 0. It returns the number of
/// bytes written to in the `dest` buffer.
///
/// # Panics
///
/// This function will panic if the `dest` buffer is not large enough for the
/// encoded message. You can calculate the size the `dest` buffer needs to be
/// with the `max_encoding_length` function.
...
/// Encodes the `source` buffer into the `dest` buffer using an arbitrary
/// sentinel value.
///
...
/// value. This will ensure that the sentinel value doesn't show up in the
/// encoded message. See the paper "Consistent Overhead Byte Stuffing" for
/// details.
...
impl<'a> CobsDecoder<'a> {
    /// Create a new streaming Cobs Decoder. Provide the output buffer for the
    /// decoded message to be placed in
    ...
    ///
    /// NOTE: Sentinel value must be included in the input to this function for
    /// the decoding to complete
    pub fn feed(&mut self, data: u8) -> Result<Option<usize>, usize> {
        ...
        let (ret, state) = match (&self.state, data) {
            ...
            // Currently Idle, received a byte indicating the next 255 bytes
            // have no zeroes, so we will have 254 unmodified data bytes, then
            // an overhead byte
            (Idle, 0xFF) => (Ok(None), GrabChain(0xFE)),

            // Currently Idle, received a byte indicating there will be a zero
            // that must be modified in the next 1..=254 bytes
            ...
            // We have reached the end of a data run, and we will expect `n`
            // data bytes unmodified, followed by a sentinel byte that must be
            // modified
            ...
            // When we have errored or successfully completed decoding a
            // message, latch this state to prevent confusion
...
// This needs to be a macro because `src` and `dst` could be the same or
// different.
...
/// # Failures
///
/// This will return `Err(())` if there was a decoding error. Otherwise, it will
/// return `Ok(n)` where `n` is the length of the decoded message.
///
/// # Panics
///
/// This function will panic if the `dest` buffer is not large enough for the
...
///
/// This is the same function as `decode`, but replaces the encoded message with
/// the decoded message instead of writing to another buffer.
...
///
/// This is done by XOR-ing each byte of the source message with the chosen
/// sentinel value, which transforms the message into the same message encoded
/// with a sentinel value of 0. Then the regular decoding transformation is
/// performed.
